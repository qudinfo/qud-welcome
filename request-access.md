...
<script>
  ...
  document.getElementById("qudForm").addEventListener("submit", async (e) => {
    ...
    try{
      ...
      msg.textContent = "✅ Заявка отправлена. Сейчас Вы находитесь в очереди. QUD внимательно отслеживает торговую систему кандидатов для этого требуется использовать много ресурсов. На вашу почту от qatequd@proton.me через несколько дней поступит доступ к тесту.";
      e.target.reset();
    } catch(err){
